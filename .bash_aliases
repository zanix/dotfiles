# BASH Aliases
# shellcheck disable=SC2148,SC2312
...
# Set the default git editor to the default editor
...
alias glog="git log --pretty=\"%C(Yellow)%h %C(reset)%ad (%C(Green)%cr%C(reset)) %C(Cyan)%an%x09: %C(reset)%s\" --date=format:%Y-%m-%d\ %H:%M:%S"
...
function lsb() {
	local cols="NAME,LABEL,TYPE"

	# Check for mount point column (MOUNTPOINTS is newer than MOUNTPOINT)
	if lsblk --help | grep -q "MOUNTPOINTS"; then
			cols="${cols},MOUNTPOINTS"
	elif lsblk --help | grep -q "MOUNTPOINT"; then
			cols="${cols},MOUNTPOINT"
	fi

	cols="${cols},FSTYPE,SIZE"

	# Check for filesystem usage column
	if lsblk --help | grep -q "FSUSE%"; then
			cols="${cols},FSUSE%"
	fi

	cols="${cols},MAJ:MIN,RM,RO,UUID"

	lsblk --fs --perms --output "${cols}" | grep -v '/snap'
}

# Alias to fuzzy find files in the current folder(s), preview them, and launch in an editor
# Link: https://github.com/junegunn/fzf
if [[ -x "$(command -v fzf)" ]] && [[ -n ${batname+x} ]]; then
...
# Flush DNS cache
# Link: https://www.tecmint.com/flush-dns-cache-in-linux/
...
elif [[ -x "$(command -v nscd)" ]]; then
	alias flushdns='sudo nscd --invalidate=hosts && echo "DNS cache flushed"'
elif [[ -x "$(command -v dnsmasq)" ]]; then
	alias flushdns='sudo systemctl restart dnsmasq && echo "DNS cache flushed"'
elif [[ -x "$(command -v named)" ]]; then
	alias flushdns='sudo rndc flush && echo "DNS cache flushed"'
elif [[ -x "$(command -v host)" ]]; then
	alias flushdns='sudo host -v -c -t txt . && echo "DNS cache flushed"'
else
	alias flushdns='echo "No DNS cache flushing command found"'
fi
